...
def apply_dl_mask(img, crop_corners, mask):
    x1 = crop_corners[0,0]
    x2 = crop_corners[1,0]
    y1 = crop_corners[0,1]
    y2 = crop_corners[2,1]
...
def apply_splines(img, crop_corners, spline, color):
    height = img.shape[0]
    width = img.shape[1]
    output = copy.deepcopy(img)
...
class rope_detect:
    ...
    def gp_estimation(self, img, l=0.1, debug=False):
        ...
        self.masked_img = apply_dl_mask(img, self.crop_corners, dl_mask)
        ...
        if debug:
            ...
            for i in range(len(resp1.tck)):
                spline = eval_spline(resp1.tck[i], self.crop_corners, (640,480))
        ...
        for i in range(len(resp1.tck)):
            spline = eval_spline(resp1.tck[i], self.crop_corners, (640,480))
            checked = check_spline(spline, self.rod_info.box2d, l/self.scale)
            if checked is None:
                continue
            else:
                break
